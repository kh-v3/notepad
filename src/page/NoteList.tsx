import React from 'react';

import sample1 from 'assets/img/sample/sample1.jpg';
import sample2 from 'assets/img/sample/sample2.png';
import sample3 from 'assets/img/sample/sample3.jpg';
import sample4 from 'assets/img/sample/sample4.jpg';
import sample5 from 'assets/img/sample/sample5.jpg';
import sample6 from 'assets/img/sample/sample6.jpg';
import sample7 from 'assets/img/sample/sample7.jpg';

function NoteList() {
  return (
    <article className="post__grid">
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목</div>
          <div className="post__thumbnail">
            <img src={sample1} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 
            내용내뇨용 내용내뇨용 내용내뇨용 
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample2} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample3} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample4} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample5} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample6} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample7} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample3} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample3} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample3} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample3} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
      <div className="post__cell">
        <div className="post">
          <a href="/detail" className="post__link"><span className="hidden-text">상세보기</span></a>
          <div className="post__title">제목제목제목</div>
          <div className="post__thumbnail">
            <img src={sample3} alt="" />
          </div>
          <div className="post__content">
            내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용내뇨용 내용
          </div>
          <div className="post__regist-date">2022-09-25</div>
        </div>
      </div>
    </article>
  );
}

export default NoteList;